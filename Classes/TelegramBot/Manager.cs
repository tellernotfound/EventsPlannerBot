...
public class Manager
{
	...
	public Manager()
	{
		...
		if (token == "YOUR_BOT_TOKEN")
		{
			...
			throw new TokenNotFoundException("Default token detected!");
		}
		...
		switch (command)
		{
			...
			case "/photo":
				...
				else
				{
					...
					await bot.SendPhoto(msg.Chat, fileStream, caption: "Read https://EventsPlannerBots.github.io/book/");
				}
				break;
			case "/inline_buttons":
				await bot.SendMessage(msg.Chat, "Inline buttons:", replyMarkup: new InlineKeyboardButton[][] {
					["1.1", "1.2", "1.3"],
					[("WithCallbackData", "CallbackData"), ("WithUrl", "https://github.com/EventsPlannerBots/Telegram.Bot")]
				});
				break;
			case "/keyboard":
			...
			case "/remove":
				...
				break;
			case "/poll":
				await bot.SendPoll(msg.Chat, "Question", ["Option 0", "Option 1", "Option 2"], isAnonymous: false, allowsMultipleAnswers: false);
				break;
			case "/reaction":
				await bot.SetMessageReaction(msg.Chat, msg.Id, ["❤"], false);
				break;
			case "/dice":
				await bot.SendDice(msg.Chat);
				break;
		}
	}
	...
	private async Task OnUpdate(Update update)
	{
		switch (update.Type)
		{
			case UpdateType.CallbackQuery:
				if (update.CallbackQuery != null) await OnCallbackQuery(update.CallbackQuery);
				break;
			case UpdateType.PollAnswer:
				if (update.PollAnswer != null) await OnPollAnswer(update.PollAnswer);
				break;
			case UpdateType.InlineQuery:
				if (update.InlineQuery != null) await OnInlineQuery(update.InlineQuery);
				break;
			default:
	...
	private async Task OnInlineQuery(InlineQuery query)
	{
		await bot.SendDice(query.From.Id);
	}
}
